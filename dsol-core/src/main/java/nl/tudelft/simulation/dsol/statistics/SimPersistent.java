        ...
        implements StatisticsInterface<T>
{
     ...
     * @throws RemoteException on network error for one of the listeners
     */
    public SimPersistent(final String description, final SimulatorInterface<T> simulator, final EventProducerInterface target,
        ...
        else if (isActive())
        {
            if (event instanceof TimedEvent<?>)
            {
                ...
                if (timedEvent.getTimeStamp() instanceof Number)
                {
                    ...
                    super.notify(new TimedEvent<T>(timedEvent.getType(), timedEvent.getSourceId(), timedEvent.getContent(),
                            ((T) timedEvent.getTimeStamp())));
                }
                else
                {
                    super.notify(event);
